...
- 共有によるタスク追加(他のアプリの共有メニューからOboetotteを選ぶと、確認画面無しで「タスク(年/月/日 時:分)」という名前の新規タスクを即座に作成。共有した写真・ファイル〈複数選択時はまとめて1タスクに〉はそのタスクの添付ファイルとして保存され、期限は他のタスク登録と同様デフォルトで1時間後に設定される)
